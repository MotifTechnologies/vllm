...
def _build_vllm_dp_server_env(
    ...
    # Routable API-server port for this rank (== child_args.port). Lets the
    # rank's KV-events publisher render a per-rank ``{port}`` topic that a
    # topic-attributing consumer can map back to this exact endpoint.
    child_port = args.port + local_rank
    return {
        device_env: visible_devices,
        "VLLM_KV_EVENTS_ENDPOINT_PORT": str(child_port),
    }
