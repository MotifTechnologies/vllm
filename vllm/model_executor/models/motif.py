...
class MotifAttention(nn.Module):
    def __init__(
        ...
        self.total_num_noise_heads = num_noise_heads
        self.num_noise_heads = self.total_num_noise_heads // tp_size

        if hasattr(config, "num_noise_heads") and not hasattr(config, "head_dim"):
            raise ValueError(
                "When num_noise_heads is set in the config, head_dim must also be set."
            )
        ...
        self.o_proj = RowParallelLinear(
            input_size=2
            * (self.total_num_heads - self.total_num_noise_heads)
            * self.head_dim,
            output_size=hidden_size,
            bias=bias_o_proj,
            quant_config=quant_config,
        ...
        params = {
            "grouped_differential_attention_config": {
                ...
                "num_q1_heads": self.num_heads - self.num_noise_heads,
                "num_q2_heads": self.num_noise_heads,
                "subln": self.subln,
            }
        }

        gda_err_msg = (
            'Set VLLM_ATTENTION_BACKEND="GROUPED_DIFF_ATTN" '
            "to enable Grouped Differential Attention."
        )
        try:
            self.attn = Attention(
                self.num_heads,
    ...
    def forward(
        ...
        output_shape = (
            ...
            2 * (self.num_heads - self.num_noise_heads) * self.head_dim,
        )
        attn_output = self.attn(q, k, v, output_shape=output_shape)
...
class MotifDecoderLayer(nn.Module):
    def __init__(
        ...
        def safe_half(x: int) -> int:
            if x % 2 != 0:
                raise ValueError(f"{x} is not divisible by 2")
            return x // 2
        ...
        self.self_attn = MotifAttention(
            ...
            num_noise_heads=getattr(
                config, "num_noise_heads", safe_half(config.num_attention_heads)
            ),
...
# Motif model uses grouped differential attention
